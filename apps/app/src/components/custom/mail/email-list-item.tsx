...
export function EmailListItem({ mail, isSelected, onSelect }: EmailListItemProps) {
  return (
    <button
      ...
      className={`mb-2 w-full cursor-pointer rounded-xl border px-3 py-3 text-left transition-all duration-300 ${
        isSelected
    ...
    >
      ...
      <div className="mb-1 flex items-center justify-between gap-2">
        <p
          className={`text-sm ${
            mail.unread ? "font-semibold text-zinc-900 dark:text-zinc-100" : "font-medium text-zinc-700 dark:text-zinc-300"
          }`}
        >
          {mail.sender}
        </p>
      ...
      <p
        className={`truncate text-sm ${
          mail.unread ? "font-semibold text-zinc-800 dark:text-zinc-100" : "text-zinc-700 dark:text-zinc-300"
        }`}
      >
        {mail.subject}
      </p>
